---
version: alpha
name: Minimalist E-Ink Design System
description: A minimalist, e-ink-inspired design system for any product. Surfaces alternate between charcoal-paper (`#1C1B18` light/dark mode) and paper-white (`#ffffff`/`#F2EFE7`) bands. Core colors are Ink Black (`#000000` on light, paper-white on dark), Body Grey (`#959494` on light, `#A39E93` on dark), and the three-stop brand gradient (Orange `#fc4c02`, Magenta `#ef2cc1`, Periwinkle `#bdbbff`). Text sits in Ink on light surfaces and Paper-White on dark; secondary text in Body Grey. All styling—from buttons to charts to tables—routes through these core colors. The one decorative element is the three-color gradient, usable anywhere at any scale. Knit together by a custom display sans (or Inter with `ss01`) and an uppercase mono eyebrow face.
...
The visual model is an e-ink tablet — a matte reading device, not a glowing app. Surfaces alternate: charcoal-paper bands on dark (`#1C1B18` + paper-white text `#F2EFE7`), paper-white bands on light (`#ffffff` + ink text `#000000`). All secondary text runs in Body Grey. Data, typography, and hairline rules do all the work; the three-color orange-magenta-periwinkle gradient is the single piece of decorative chrome and can appear anywhere. No illustration system beyond the gradient. Minimal, intentional, readable.
...
Surfaces alternate aggressively: a `{colors.canvas-dark}` (`#1C1B18`) charcoal-paper band for hero / research / "Grounded in cutting-edge research" — followed by `{colors.canvas}` (white) for product, pricing, and testimonials, with `{colors.hairline}` reserved for table-header rows and toggle backgrounds. Pastel `{colors.accent-mint}` tinted stat tiles break up the white middle. Cards are universally lightly rounded (`{rounded.sm}` 4 px) with hairline borders — never floating with shadows.
...
- A three-color brand gradient (`{colors.accent-orange}` → `{colors.accent-magenta}` → `{colors.accent-periwinkle}`) is the entire decorative system — used anywhere (hero ribbon, accents, data series, dividers) and never reduced to a single-colour swatch.
...
- A massive wordmark banner at the very bottom of every page, set in `{typography.display-xxl}` and tinted nearly-into-the-canvas (`{colors.hairline}`), as a sign-off and footer separator.
...
- **Canvas Dark / Charcoal Paper** (`{colors.canvas-dark}` — `#1C1B18`): The brand's dark hero surface — a warm charcoal rather than navy or pure black, tuned to read like ink-saturated paper. Appears on `hero-band-dark` and `research-band-dark`.
- **Hairline** (`{colors.hairline}` — `#ebebeb`): 1 px dividers on light surfaces — table rows, card chrome, badge borders.
- **Hairline on Dark** (`{colors.hairline-dark}` — `rgba(242, 239, 231, 0.12)`): 1 px dividers and card borders on `{colors.canvas-dark}` surfaces — translucent paper-white, never pure white, so hairlines look pressed into the charcoal rather than glowing on top of it.
- **Surface Dark Soft** (`{colors.surface-dark-soft}` — `#2A2823`): A slightly lifted warm-charcoal fill used inside dark-band cards, ghost buttons, and on-dark badges — one shade of graphite above the canvas, no blue cast.
...
- **On Dark / Paper White** (`{colors.on-dark}` — `#F2EFE7`): All primary text on `{colors.canvas-dark}` surfaces — a warm paper off-white, not `#ffffff`. The ink/paper inversion is the whole point: dark mode reads as white ink on charcoal paper.
- **On Dark Muted** (`{colors.on-dark-muted}` — `#A39E93`): Secondary text, tickers, timestamps, and mono eyebrows on dark surfaces — warm grey, pre-blended from paper-white at ~60 % over the charcoal canvas.
...
### Brand Gradient
The brand's signature decoration is a three-stop gradient drawn from `{colors.accent-orange}` → `{colors.accent-magenta}` → `{colors.accent-periwinkle}`, applied as the only piece of decorative chrome and usable anywhere — hero ribbon, accent lines, data series, button fills, underlines. Treat the gradient as one unified object — do not crop it down to a single colour, do not reorder the stops, and do not add a fourth stop. Works at any scale from icon to full-width.

## Minimalism & E-Ink Principles

The whole system should feel like an e-ink tablet displaying market data — a reMarkable or Kindle rendering a broadsheet, not a glowing dashboard. Minimalism here is subtractive: if an element is not data, a label, or a rule, it should probably not exist.

### Principles
- **Grayscale first.** Design every screen to work in pure grayscale, then add color back only where it carries meaning (positive/negative tickers, the single optional gradient ribbon). If a screen fails in grayscale, it has too much decoration.
- **Ink or paper — nothing in between glows.** Surfaces are matte fills from the graphite scale; text is ink. No gradients on UI chrome, no glassmorphism, no blur, no glow, no colored shadows. The one brand gradient stays confined to the hero ribbon, and even that is optional per page.
- **One accent per viewport.** At most one colored moment visible at a time (a ticker delta, the live-status dot). Everything else earns attention through weight and spacing, not hue.
- **Hairlines carry all structure.** 1 px rules divide sections, tables, and cards the way a newspaper divides columns. No card shadows, no elevated panels — a bordered rectangle on a flat sheet is the maximum container treatment.
- **Static by default.** E-ink does not animate. Transitions are instant or near-instant (≤ 100 ms opacity at most); no parallax, no scroll-triggered motion, no skeleton shimmer. A page refresh should feel like an e-ink page turn: the new state simply appears.
- **Density over chrome.** Prefer a tight, well-ruled data table to a grid of padded cards. Whitespace is generous *between* content blocks and tight *within* them — the broadsheet rhythm.
- **Monochrome imagery.** Any photography or illustration renders grayscale (or 1-bit dithered for decorative uses). Charts default to ink-on-paper line work: single-weight strokes, no area fills, no more than one series color.
- **Reduce, then reduce again.** Icons only where a label cannot work; borders only where spacing cannot separate; color only where grayscale cannot communicate.

### What this removes from the base system
- Pastel `{colors.accent-mint}` stat tiles are retired in favor of ruled, uncolored stat rows; `{colors.accent-mint}` survives only as a rare hero secondary-CTA fill, if at all.
- The soft drop shadow (Level 3) shrinks to the floating chat-launcher orb exclusively — and even there a hairline border is preferred.
- Decorative depth effects (layered translucent shapes) are dropped; the hero may run entirely type-on-charcoal with no ribbon.

## Dark Mode — "Charcoal Paper"

Dark mode is not an inversion switch; it is a material change. The surface behaves like charcoal paper — warm, matte, slightly toothy — and everything set on it behaves like ink. Think of it as the e-ink tablet's night palette: the same static, matte, grayscale-first discipline as light mode, printed in reverse.

### Principles
- **Warm charcoal, never navy, never pure black.** The canvas is `{colors.canvas-dark}` `#1C1B18` — a near-black with a brown-grey bias (roughly 8 % lightness, hue ~45°). No blue-channel dominance anywhere on the dark surface; a blue cast reads as "OLED app", not paper.
- **Paper-white ink, never `#ffffff`.** Primary text sits at `{colors.on-dark}` `#F2EFE7`; secondary text at `{colors.on-dark-muted}` `#A39E93`. Pure white on charcoal glares; off-white settles into the surface like ink absorbed by fibre.
- **Hairlines are pressed, not glowing.** Dividers and card borders use `{colors.hairline-dark}` `rgba(242, 239, 231, 0.12)` — translucent paper-white over charcoal — so lines look debossed into the sheet.
- **Elevation by graphite step, not shadow.** A lifted surface moves one shade up the graphite scale (`{colors.surface-dark-soft}` `#2A2823`); shadows are reserved for genuinely floating elements and are tinted with the canvas itself (`rgba(20, 19, 16, 0.35)`).
- **Optional grain.** A very-low-opacity monochrome noise texture (≤ 3 % opacity) over `{colors.canvas-dark}` sells the paper tooth. It must be imperceptible at reading distance — a texture you feel, not see.
- **Accents stay inky.** Positive/negative ticker colors and the brand gradient survive dark mode but are desaturated ~10–15 % so they read as pigment on paper rather than neon on glass. Semantic green ≈ `#5FA97C`, semantic red ≈ `#C96A5E` on charcoal.

### Graphite Scale (dark surfaces, darkest → lightest)
| Step | Value | Use |
|---|---|---|
| Canvas | `#1C1B18` | Page background, hero band, nav. |
| Surface Soft | `#2A2823` | Cards, ghost buttons, badges, input fills. |
| Surface Raised | `#33312B` | Hover states, active nav rows, code-editor chrome. |
| Hairline | `rgba(242, 239, 231, 0.12)` | Borders, dividers, table rules. |

### Polarity Flips
- `button-primary` flips to a paper-white fill (`{colors.on-dark}`) with charcoal text (`{colors.canvas-dark}`) — the ink/paper inversion of the light-mode black pill. Still exactly one per viewport.
- `button-ghost-on-dark` keeps `{colors.surface-dark-soft}` fill with `{colors.on-dark}` text; on hover it steps to Surface Raised, never to a border glow.
- `text-input` on dark uses Surface Soft fill, `{colors.hairline-dark}` border, `{colors.on-dark}` text.
- The footer wordmark banner tints toward Surface Raised so it reads as a graphite stencil rubbed into the sheet.
...
| `{typography.display-xl}` | 40px | 500 | 48px | -0.8px | Section headlines. |
...
- **Footer wordmark banner**: scales fluidly — the giant wordmark stays edge-to-edge regardless of viewport.
...
| Level 3 — Soft Drop | `rgba(20, 19, 16, 0.35) 0px 4px 10px 0px` — a barely-perceptible shadow tinted with the charcoal canvas, warm and neutral (no blue cast). | Floating elements (the chat-launcher orb, sticky-bottom nav row when one appears). |
...
- **Wordmark banner as terminal depth**: the giant wordmark letters at the bottom are technically inside `{colors.canvas}` but tinted toward `{colors.hairline}` so they read as a faint stencil, giving the page a final "you have arrived" sign-off.
...
**`testimonial-card`** — the 3-up testimonial card.
...
**`article-card`** — the 2-up article card.
...
**`footer-wordmark-banner`** — the massive wordmark at the bottom of every page.
...
- Use the brand gradient (`{colors.accent-orange}` → `{colors.accent-magenta}` → `{colors.accent-periwinkle}`) anywhere — hero ribbon, accent underline, data series, button fill, divider. The gradient is the brand chrome and works at any scale from icon to full-width. Never reduce to a single-colour swatch.
...
- Render the giant wordmark banner at the bottom of every long page in `{typography.display-xxl}`, tinted toward `{colors.hairline}` so it reads as a stencil — not as a heavy footer title.

- Design grayscale-first and static-first: hairline rules over shadows, instant state changes over animation, at most one colored accent per viewport — the e-ink discipline.

### Don't
- Don't add motion, glow, blur, or gradient chrome to UI surfaces — nothing on an e-ink sheet shimmers.
