...
import { Bookmark, Bell } from "lucide-react-native";
import {
  usePortfolio,
  useTickerHistory,
  useTickerNews,
...
/**
 * Reusable stock detail content — chart, position card, key stats, news.
 * Used by both the /stock/[ticker] route and the tablet dashboard right panel.
 * No navigation chrome — callers provide back buttons etc.
 */
export function StockDetailPanel({
  ...
  // Use panelWidth when embedded in a panel, otherwise full screen width minus padding
  const chartWidth = (panelWidth ?? screenWidth) - 64;
  ...
  return (
    <>
      ...
      {/* Price header row with watch/alert actions */}
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        <View style={{ marginTop: 12, opacity: historyQuery.isFetching ? 0.5 : 1 }}>
          <PerformanceChart data={chartData} range={range} width={chartWidth} />
        </View>
      </Card>
